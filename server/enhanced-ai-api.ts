...
class EnhancedAIService {
  ...
  private initializeAnthropic() {
    try {
      if (process.env.ANTHROPIC_API_KEY) {
        ...
        console.log('🤖 Claude 4.0 Sonnet API initialized successfully');
      } else {
        console.log('⚠️ ANTHROPIC_API_KEY not found - using enhanced local generation');
      }
    } catch (error) {
      console.warn('Failed to initialize Anthropic API:', error);
    }
  }

  async generateProject(request: AIGenerationRequest): Promise<AIGenerationResponse> {
    console.log(`🚀 Starting AI project generation with Claude 4.0 Sonnet...`);
    
    try {
      // Try Claude 4.0 Sonnet first if available
      if (this.isConfigured && this.anthropic) {
        return await this.generateWithClaude4(request);
      }
      
      // Fall back to enhanced local generation
      return await this.generateWithEnhancedLocal(request);
  ...
  private async generateWithClaude4(request: AIGenerationRequest): Promise<AIGenerationResponse> {
    if (!this.anthropic) throw new Error('Anthropic not initialized');

    const systemPrompt = `You are an expert full-stack developer and AI assistant. Your task is to generate complete, production-ready applications based on user requests. 

Key requirements:
1. Generate COMPLETE, functional applications - not just templates or examples
...
8. Format code with proper structure and comments

For each project, return:
- A detailed explanation of what you built
- Complete file structure with all necessary files
- Working, production-ready code

Format your response to include file blocks like this:
\`\`\`html:index.html
...
\`\`\``;
    ...
    console.log('🧠 Sending request to Claude 4.0 Sonnet...');
    ...
    const files = this.extractFilesFromClaudeResponse(aiResponse);

    console.log(`✅ Claude 4.0 Sonnet generated ${Object.keys(files).length} files`);
...
- Include proper error handling`;
  ...
  private extractFilesFromClaudeResponse(response: string): Record<string, { content: string; language: string }> {
    ...
    // If no files extracted, create a default based on content
    if (Object.keys(files).length === 0) {
      files['index.html'] = {
        content: this.generateDefaultHTML(response),
        language: 'html'
      };
    }
